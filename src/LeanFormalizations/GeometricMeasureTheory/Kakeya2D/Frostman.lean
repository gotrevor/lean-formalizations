...
`d` charging the Kakeya set `S` exists.* `hausdorffMeasure_ne_zero_of_frostman` discharges the
implication; building the measure from the K4 content bound is what remains. -/
def FrostmanMeasureExists (S : Set Plane) (d : ℝ) : Prop :=
  ∃ (μ : Measure Plane) (ε : ℝ≥0∞), 0 < ε ∧
    (∀ s : Set Plane, Metric.ediam s ≤ ε → μ s ≤ Metric.ediam s ^ d) ∧ μ S ≠ 0
...
theorem hausdorffMeasure_ne_zero_of_frostmanExists {S : Set Plane} {d : ℝ}
  ...
  obtain ⟨μ, ε, hε, hfrost, hpos⟩ := h
  exact hausdorffMeasure_ne_zero_of_frostman μ ε hε hfrost hpos
