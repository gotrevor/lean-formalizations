/-
# Prime Number Theorem port — `WeakPNT''` (ψ ∼ x), the discharge target

Minimal slice of `PrimeNumberTheoremAnd/Consequences.lean` (Apache-2.0) carrying the chain
`WeakPNT` (in the ported `Wiener`) → `WeakPNT'` → `WeakPNT''`.  `WeakPNT''` is exactly the statement
of the cited `axiom weakPNT : Chebyshev.psi ~[atTop] (·)` that the flagship no-three-in-line headline
`maxNoThreeInLine_ge_three_halves_sub` rests on.  This is the **discharge** of that axiom into a
machine-checked proof (modulo any `sorry` carried transitively from `Wiener`; verified by
`#print axioms`).

Only the declarations actually needed for `WeakPNT''` are included — the FKS2 error-bound layer
(`Defs`) is not on this path and is omitted.  `@[blueprint]`/`Architect` stripped.
-/
...
/-- An alternate form of the Weak PNT: `ψ(x) ∼ x` as `x → ∞`. -/
