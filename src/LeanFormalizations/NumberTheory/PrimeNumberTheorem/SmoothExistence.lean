/-
# PNT port — SmoothExistence brick

Port of `PrimeNumberTheoremAnd/SmoothExistence.lean` (Apache-2.0): a smooth nonnegative
bump function supported in `[1/2,2]` with `∫ ν(x)/x = 1`, plus the smooth-Urysohn helper.
A leaf prerequisite of the Wiener tower (`weakPNT` discharge). `@[blueprint]` attrs and the
`Architect` import stripped; `Function.support_id'` supplied by the local Support patch.
-/
