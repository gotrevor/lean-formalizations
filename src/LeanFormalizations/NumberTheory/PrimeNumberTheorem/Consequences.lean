/-
# Prime Number Theorem port — `Consequences`

Full port of `PrimeNumberTheoremAnd/Consequences.lean` (Apache-2.0), built on the in-repo Wiener
tower (`WeakPNT`).  Carries the classic PNT in all standard forms — `WeakPNT''` (ψ∼x),
`chebyshev_asymptotic` (θ∼x), `pi_alt'` (π(x)∼x/log x), `nth_prime_asymp` (pₙ∼n log n) — all
axiom-clean against our mathlib `v4.29.1`.  `@[blueprint]`/`blueprint_comment`/`Architect` stripped;
the `Log/Basic` patch is supplied by the local `Asymptotics` module.
-/
